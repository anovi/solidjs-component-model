...
import { expectUncaughtException, expectUnhandledRejection, sleep } from './test-kit';
...
import { ComponentModel, TerminalLogger, MachineMalformed, StateChart } from '../src';
...
describe('component-model', () => {
            ...
            assert.deepEqual(
                parent.toJSON(),
                {
                    ...
                    data: {
                        counter: 0,
                        some: 'info',
                        children: [{ _id: parent.data.children[0]._id, state: 'default', name: "Child", status: 'active', data: { some: 'info'} }]
                    }
                } as any
    ...
    describe('child state', () => {
        it('moves to child state', async () => {
            const model = new ModelWithChildStateNodes();
            model.start();
            assert.ok(model.defaultStateEntryHookTriggered, 'should trigger entry hook of inital state');
    ...
    describe('delayed events', () => {
        ...
        it('cancels all scheduled actions of the exited state, not just the transition one', async () => {
            ...
            await sleep(60);
            assert.equal(model.data.delayedFired, false);
            assert.equal(model.data.secondScheduleFired, false, 'second schedule should be cancelled on exit too');
            model.stop();
        })
        ...
        it('does not cancel scheduled actions of a sibling state', async () => {
            ...
            assert.ok(model.matches('Third'));
            await sleep(60);
            assert.equal(model.data.delayedFired, false, 'schedules of exited Second state should be cancelled');
            assert.equal(model.data.secondScheduleFired, false, 'schedules of exited Second state should be cancelled');
            assert.equal(model.data.thirdEntryScheduleFired, true, 'schedule of the current Third state should still fire');
            model.stop();
        })
    ...
    describe('Restoring models from snapshots', () => {
        ...
        it('it restores a model with children and TOP node invoke', async () => {
            ...
            assert.equal(newParent.data.counter, 1, 'the counter value is restored from snapshot');
            ...
            await sleep(310);
            assert.equal(newParent.data.counter, 2, 'should invoke new TOP observable, and start counding from 0');
        })
        ...
        it('it restores a model without no-explicit children', async () => {
            ...
            assert.deepEqual(parent.getChildrenData(), ['from parent', 'from parent', 'from parent'], 'child models should receive a message');
            ...
            assert.equal(newParent._id, parent._id);
            assert.equal(newParent.status, 'active');
            assert.equal(newParent.childrenLength, 0, 'must not have children because they are not in snapshot');
            assert.ok(newParent.matches('some'), 'should have restored state');
        })
        ...
        it('stops restored children', async () => {
            ...
            assert.lengthOf(newParent.data.children, 3);
            // assert.ok(newParent instanceof ParentModel);
            newParent.data.children.forEach(child => assert.equal(child.status, 'active'));

            // Stop the model
            newParent.stop();
            newParent.data.children.forEach(child => assert.equal(child.status, 'stopped'));
        })
        ...
        it('caches and restores with custom snapshot function', async () => {
            ...
            const newParent = CustomizedParentModelMachine.fromPersistedSnapshot(snapshot);
            newParent.start();
