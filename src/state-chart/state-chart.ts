...
> {
  ...
  static create<
    TContext = any,
    E extends Event = Event,
    TConfig extends StateChartConfig<TContext, E> = StateChartConfig<
      TContext,
      E
    >,
  ...
  getEventHandlers(
    ...
    if (node.always)
      return node.always as Transition<TModel, any>[] | undefined;
  }
  ...
  createRuntime(
    context: TModel
  ): Interpreter<TModel, E, StateChart<any, any, TConfig>> {
  ...
  #validateHandlers() {
    this.#toValidate!.every(handlers =>
  ...
  #makeNode(
    ...
    if (config.always) {
      const handlers = Array.isArray(config.always)
        ? config.always
        : [config.always];
...
export class Interpreter<
  TModel extends ExecutionContext = any,
  E extends Event = Event,
  TChart extends StateChart<any, any, any> = StateChart<any, any, any>,
> {
  constructor(
    private context: any,
    public readonly chart: TChart
  ) {}
  ...
  *transition(
  ...
  getMostSpecificHandler(
    pathString: string,
    event?: E
  ): Transition<TModel, any> | Error | undefined {
