...
import { batch, createSignal, type Accessor, type Setter, untrack } from "solid-js";
import { createStore, unwrap, type SetStoreFunction, type Store } from "solid-js/store";
import { Subject, type Observer, type Observable, type Unsubscribable } from "./observable";

import { Scheduler } from "./scheduler";
import { type AnyModelData, type EventType, type Invoke, type Snapshot, type FrameworkConfig, type Status, InternalEventName, type InvokedNext, type InvokedError, type InvokedDone, type InternalEvent, type AnyModel } from "./types";
import { StateChart, Interpreter, type Event, type EventName, type StateChartConfig, type Transition, type TransitionStep, MachineMalformed } from "./state-chart";
...
import type { Eventless } from "./events";
import { Stack } from "./stack";


type SendApi<E extends { type: string }> = {
    [K in EventName<E>]:
        (payload?: Omit<Extract<E, { type: K }>, "type">) => void;
};
...
interface ComponentModelConstructor {
    ...
    fromSnapshot: (snapshot: unknown, ownerCtor?: ModelCtorWithChildren) => AnyComponentModel;
...
> implements Model<Data, E, Emitted, DoneData> {
    ...
    constructor(ctx: Data) {
        ...
        const stateChartSetup = (
            this.constructor as ModelConstructor<Model<Data, E, Emitted, DoneData>, E|InternalEvent>
        ).chart;
        if (stateChartSetup) {
    ...
    static fromJSON<TThis extends new (...args: any[]) => AnyComponentModel>(
        ...
        if (!ctor.isModelSnapshot(snapshot)) throw Error(`${snapshot} is not a model's snapshot.`);
        return ctor.fromSnapshot(snapshot) as InstanceType<TThis>;
    }

    static fromPersistedSnapshot<TThis extends new (...args: any[]) => AnyComponentModel>(
        this: TThis,
        snapshot: unknown
    ): InstanceType<TThis> {
        const ctor = this as unknown as ComponentModelConstructor;
    ...
    dispatch(event: E): void {
        event = unwrap(event);
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't dispatch "${event.type}" for a`);
    ...
    start() {
        if (this.status !== 'idle') return this.#warnNonActiveModel(`Can't start a`);
        ...
        if (this.stateChart) {
            ...
            this.#executeEventHandler({ type: InternalEventName.Start }, { target, reenter: this.state() !== '' });
        }
    }

    stop() {
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't stop a`);
		this.status = 'stopped';
        this.#destroy();
    ...
    protected invokeObservable<Next>(
        ...
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't invoke observable for a`);
            ...
            const sub = observable.subscribe({
                ...
                error: (error) => {
                    if (handler.error) {
                        ...
                        this.#machineMalformedInRuntime(
                            new MachineMalformed(`unhandled error in observable in "${state}"`, { cause: error, machineConfig: this.constructor }),
                            true
                        );
                        return;
                    }
                ...
                complete: () => {
                    if (handler.complete)
                        this.enqueue(() => {
                            this.#executeEventHandler(
                                { type: InternalEventName.InvokedDone, result: undefined, state },
    ...
    protected invokePromise<T>(
		...
		if (this.status !== 'active') return this.#warnNonActiveModel(`Can't invoke promise for a`);
        const state = this.state();
        this.#invoke(async ({ signal }) => {
            try {
                const result = await promise(signal);
                const event = { type: InternalEventName.InvokedDone, result, state };
                const handler = this.#getOnDoneHandler(Array.isArray(params.onDone) ? params.onDone : [params.onDone], state, event);
                if (!handler || handler instanceof MachineMalformed) {
                    return this.#machineMalformedInRuntime(
                        handler || new MachineMalformed(`error in state "${state}"`, { cause: 'No onDone handler found', machineConfig: this.constructor })
                    )
                }
                else if (handler instanceof Error) return this.#toErrorWithReason(handler);
				...
				if (params.onError) {
                    ...
                    const malformedErr = new MachineMalformed(`unhandled promise rejection in "${state}"`, { cause: error, machineConfig: this.constructor });
    ...
    protected emit(event: Emitted) {
        event = unwrap(event);
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't emit "${event.type}" in a`);
        this.enqueue(() => {
    ...
    protected schedule(handler: Transition<this, E|InternalEvent>, timeout?: number): void {
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't schedule in a`);
        if (!this.#scheduler) this.#scheduler = new Scheduler();
        this.#scheduler.schedule(
            this.#executeEventHandler.bind(this, { type: InternalEventName.ScheduledExecute, state: this.state() }, handler),
            timeout || 0,
            this.state()
        );
    }
    ...
    protected enqueue(task: () => void, parentSpan?: Span) {
        ...
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't enque in a`);
    ...
    private readonly stateChart?: Interpreter<Model<Data, E, Emitted, DoneData>, E | InternalEvent>;
    ...
    #getOnDoneHandler(
        ...
        let handler: Transition<ComponentModel<Data, any, Emitted>, any> | undefined = undefined;
        for (let i = 0; i < handlers.length; i++) {
            handler = handlers[i];
            if (handler.guard) {
                // There is no event for guard inside entry/exit effects.
				try {
					if (handler.guard.call(this as ComponentModel<Data, any, Emitted>, event as never)) return handler;
				} catch (error: unknown) {
                    return new MachineMalformed(`error in guard in state "${foundPath}"`, { cause: error, machineConfig: this.constructor })
                }
            }
            else return handler;
        }
    ...
    #invoke(effect: Invoke<E>) {
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't invoke for a`);
	...
	#destroy(): void {
        ...
        this.#queue.flush();
        if (this.#invocations) for (const [state] of this.#invocations) {
    ...
    #handleEvent(event: E): void {
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't handle "${event.type}" in a`);
        const handler = this.stateChart!.getMostSpecificHandler(this.state(), event);
        if (!handler) return;
        if (handler instanceof Error) return this.#toErrorWithReason(handler);
        this.#executeEventHandler(event, handler as Transition<this, E | InternalEvent>);
    }
    ...
    #executeEventHandler(event: E | InternalEvent, Transition: Transition<this, E|InternalEvent>): void {
        if (this.status !== 'active') return this.#warnNonActiveModel(`Can't handle "${event.type}" in a`);
        const span = this.#startTrace(
            `Event`,
            this.#isNormalEvent(event) ? { ...event } : this.#internalEventToTracer(event),
        );
        this.#logGroup();
        this.#logEvent(event);
        ...
        if (Transition?.target != null) {
            this.#transitionStepByStep(
                this.stateChart!.transition(this.state(), Transition.target, Transition.reenter),
                event
            );
        }
        ...
        const isTheSameAlwaysEffect = event.type === InternalEventName.Eventless && (event as Eventless).state === state;
        ...
        if (!isTheSameAlwaysEffect) {
            ...
            else if (always) this.#executeEventHandler({ type: InternalEventName.Eventless, state: state }, always);
        }
    ...
    #transitionStepByStep(
        generator: Generator<TransitionStep<Model<Data, E, Emitted, DoneData>, E | InternalEvent>>,
        event: E | InternalEvent,
        parentSpan?: Span
    ) {
        ...
        for (const step of generator) {
            if (step.exit) {
                ...
                const span = effect && this.#startSpan('Exit', { parent: parentSpan?.context(), attributes: { state: step.path} });
				...
				this.#stateSetter(step.path);
                const effect = step.effect;
                const span = effect && this.#startSpan('Entry', { parent: parentSpan, attributes: { state: step.path } });
        ...
        untrack(() => batch(() => {
            while (fn) {
                try {
                    ...
                    const err = new EffectFailed(`${type} effect in "${state}" state failed`, { cause: error })
					this.#logError('err', err);
					console.error(err);
                }
    ...
    #jsonModel(value: AnyComponentModel, name: string = ''): Snapshot<string, Data> {
        return {
            _id: value._id,
            state: value.state(),
            name: name,
    ...
    #jsonData(value: unknown, owner: AnyComponentModel): unknown {
        if (value instanceof ComponentModel) {
            const childObject =
                ((value.parent as AnyComponentModel).constructor as ModelCtorWithChildren).childTypes;
    ...
    private static fromSnapshot(
        ...
        if (this === ComponentModel) throw new Error('You need to extend you model from ComponentModel class.')
        const name = snapshot.name;

        let inst: AnyComponentModel;

        const data = (this as typeof ComponentModel).dataFromJSON(snapshot.data, ownerCtor);

        if (ownerCtor) {
            const ctor = (this as typeof ComponentModel).getChildCtor(ownerCtor, name);
            if (!ctor) throw new Error(`Unable to find child constructor "${name}" in ${ownerCtor}`);
            // It does not matter that data is empty, we update it immediately
            inst = new ctor({}) as AnyComponentModel;
        } else {
            // It does not matter that data is empty, we update it immediately
            inst = new (this as unknown as ModelCtorWithChildren)({}) as AnyComponentModel;
        }

        inst.setData(data);
        inst.#stateSetter(snapshot.state);
        inst._id = snapshot._id as `${string}-${string}-${string}-${string}-${string}`;
        inst.status = 'idle';
        return inst;
    }

    private static dataFromJSON(value: unknown, ownerCtor: ModelCtorWithChildren | undefined): unknown {
        if (this.isModelSnapshot(value)) {
            return (this as typeof ComponentModel).fromSnapshot(value, ownerCtor || (this as unknown as ModelCtorWithChildren));
        }
    ...
    private static isModelSnapshot(value: unknown): value is Snapshot<string, AnyModelData> {
        if (!value || typeof value !== "object" || value === null) return false;

        if (!('data' in value)) return false;
        if (!value.data || typeof value.data !== "object" || value.data === null) return false;
    ...
    private static getChildCtor(constructor: ModelCtorWithChildren, name: string): ModelCtorWithChildren | undefined {
        const children = constructor.childTypes;
        if (Object.hasOwn(children, name)) return children[name] as ModelCtorWithChildren;
    }
