...
> {
  ...
  static create<TModel extends ExecutionContext, E extends Event = Event>(
  ...
  #stateTransitionActionsMap = new Map<
    string,
    Map<typeof ExitSym | typeof EntrySym, AnyAction>
  >();

  #nodeCantHave(
    config: AnyStateChartConfig,
    prop: keyof AnyStateChartConfig,
    path: string
  ): void {
    if (config[prop])
      throw new MachineMalformed(
  ...
  #addNode(nodeID: string, chart: AnyStateChartConfig, final?: boolean): void {
    if (!this.charts.has(nodeID)) {
      if (nodeID === ROOT && this.charts.has(ROOT))
  ...
  #mapHandlers(address: string, stateNode: AnyStateChartConfig) {
    if (stateNode.always) {
      const res = Array.isArray(stateNode.always)
        ? stateNode.always
        : [stateNode.always];
      this.#stateEventHandlersMap.set(address, EventlessSym, res);
    }
    const transitionEffectsInState =
      this.#stateTransitionActionsMap.get(address) || new Map();
    if (stateNode.entry)
      transitionEffectsInState.set(EntrySym, stateNode.entry);
    if (stateNode.exit) transitionEffectsInState.set(ExitSym, stateNode.exit);
    if (transitionEffectsInState.size > 0)
      this.#stateTransitionActionsMap.set(address, transitionEffectsInState);
    ...
    if ("states" in stateNode) {
      for (const name in stateNode.states) {
        ...
        this.#mapHandlers(
          address === ROOT ? name : address + "." + name,
          childStateNode
        );
      }
    }
...
> {
   ...
   */
  *transition(
    fromStateString: string,
    toStateString: string,
    reenter?: boolean
  ): Generator<TransitionStep<TModel, E>> {
    const currentState = fromStateString;
    if (reenter && currentState === toStateString) {
      yield {
        exit: true,
        path: currentState,
        effect: this.compiled.getExitEffect(currentState),
      };
      yield {
        exit: false,
        path: currentState,
        effect: this.compiled.getEntryEffect(currentState),
      };
      return;
    }
    const from = new StatePath(currentState);
    const target = new StatePath(
      this.compiled.resolveFinalState(toStateString)
    );
    ...
    if (currentState === "")
      yield {
        path: ROOT,
        exit: false,
        effect: this.compiled.getEntryEffect(ROOT),
      };
   ...
   */
  getMostSpecificHandler(
    pathString: string,
    event?: E
  ): Transition<TModel, E> | Error | undefined {
    ...
    for (const curPath of statePath.ancestors()) {
      ...
      if (handlers) {
        for (const handler of handlers) {
          if (handler.guard) {
            try {
              if (handler.guard.call(this.context, event as never))
                return handler;
            } catch (error: unknown) {
