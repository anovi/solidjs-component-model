...
async function main() {
    ...
    if (!modulePath) {
        console.error('Usage: npx tsx scripts/cli-mermaid.ts <path-to-config-module> [export-name]');
        process.exit(1);
    }
    ...
    if (exportName) {
        ...
        if (typeof exported === 'object' && exported !== null && 'states' in exported) {
            ...
            console.error(`Export "${exportName}" is a constructor without a public static config.`);
            console.error(`Please pass the config object directly or export it separately.`);
            process.exit(1);
            ...
            if (cfg && typeof cfg === 'object' && 'states' in cfg) {
                ...
                console.error(`Export "${exportName}" does not look like a state chart config or a model class.`);
                process.exit(1);
            }
        } else {
            console.error(`Export "${exportName}" does not look like a state chart config or a model class.`);
        ...
        if (mod.config && typeof mod.config === 'object' && 'states' in mod.config) {
    ...
    if (!config) {
        ...
        console.error('Expected either a `config` export, or a class with a static `config` property.');
        process.exit(1);
    }
